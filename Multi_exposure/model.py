...
class Fusion(object):
  ...
  def build_model(self):
    ...
    with tf.name_scope('g_loss'):
        self.g_loss_int=tf.reduce_mean(tf.square(self.fusion_image - self.images_ir))+1*tf.reduce_mean(tf.square(self.fusion_image - self.images_vi))
        ...
        self.g_loss_2=self.g_loss_int+50*self.g_loss_grad+1*self.g_loss_sept
  ...
  def train(self, config):
    if config.is_train:
      input_setup(self.sess, config,"Train_under")
      input_setup(self.sess,config,"Train_over")
    if config.is_train:     
      data_dir_ir = os.path.join('./{}'.format(config.checkpoint_dir), "Train_under","train.h5")
      data_dir_vi = os.path.join('./{}'.format(config.checkpoint_dir), "Train_over","train.h5")
