...
for idx_num in range(11,12):
  num_epoch=idx_num
  while(num_epoch==idx_num):
  
      reader = tf.train.NewCheckpointReader('./checkpoint/MEF.model-'+ str(num_epoch))
      ...
      with tf.Session() as sess:
          ...
          sess.run(init_op)
          data_ir=prepare_data('Test_under')
          data_vi=prepare_data('Test_over')
