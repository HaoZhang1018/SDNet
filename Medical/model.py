...
class Fusion(object):
  ...
  def build_model(self):
    ...
    with tf.name_scope('g_loss'):
        ...
        self.g_loss_2=self.g_loss_int+10*self.g_loss_grad+1*self.g_loss_sept
  ...
  def train(self, config):
    if config.is_train:
      input_setup(self.sess, config,"Train_Function")
      input_setup(self.sess,config,"Train_Structure")

    if config.is_train:     
      data_dir_ir = os.path.join('./{}'.format(config.checkpoint_dir), "Train_Function","train.h5")
      data_dir_vi = os.path.join('./{}'.format(config.checkpoint_dir), "Train_Structure","train.h5")
