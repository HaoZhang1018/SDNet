...
for idx_num in range(29,30):
  num_epoch=idx_num
  while(num_epoch==idx_num):
  
      reader = tf.train.NewCheckpointReader('./checkpoint/Medical.model-'+ str(num_epoch))
      ...
      with tf.Session() as sess:
          ...
          sess.run(init_op)
          data_ir=prepare_data('Test_Function')
          data_vi=prepare_data('Test_Structure')
